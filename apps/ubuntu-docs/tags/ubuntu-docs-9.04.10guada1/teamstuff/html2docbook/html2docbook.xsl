...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:saxon="http://icl.com/saxon" exclude-result-prefixes="xsl fo html saxon">
	...
	<xsl:template match="html:body">
		<section>
			...
			<title>
				<xsl:value-of select=".//html:h1[1]      |.//html:h2[1]      |.//html:h3[1]"/>
			</title>
	...
	<xsl:template match="html:img">
		<xsl:variable name="tag_name">
			<xsl:choose>
				<xsl:when
					test="boolean(parent::html:p) and       boolean(normalize-space(parent::html:p/text()))">
	...
	<xsl:template match="html:a[@href != ''    and not(boolean(ancestor::html:p|ancestor::html:li))]"
		priority="1">
		<para>
			<xref>
				<xsl:attribute name="linkend">
	...
	<xsl:template
		match="html:a[contains(@href,'#')    and not(boolean(ancestor::html:p|ancestor::html:li))]"
		priority="1.1">
		<para>
			<xref>
				<xsl:attribute name="linkend">
